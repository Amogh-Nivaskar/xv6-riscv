...
*Full implementation on [GitHub](https://github.com/Amogh-Nivaskar/xv6-riscv/tree/kernel-threads) and full design on the [Design Doc](https://github.com/Amogh-Nivaskar/xv6-riscv/blob/kernel-threads/docs/kernel-threads-design.md).*

---

## The Problem

Vanilla xv6 boots on 3 cores. But the way that a process is defined in xv6, it has only a single state of execution. Meaning, that at any given time, a single process can run on only one core. This is inefficient as the other cores are just idle as they can't run the same process as the execution state isn't shared between them.  
Lets take an example of a program which wants to count all the prime numbers till 200,000. In our current system, the process running this program will have to process each number one after the other, i.e. serially on one core, while the other cores are idle. But, what if we could share the execution state between the 3 cores, and thus have each core process a third of the numbers parallely, allowing for upto 3x faster performance !! A multi-threaded kernel is build to exactly this, and we have extended xv6 to grant it this capability.
...
The problem with that is you can't share one `proc` instance across CPUs. Each CPU needs its own execution context: its own register save area (trapframe), its own kernel stack, its own scheduler state. 

So we split the fields and take the ones belonging to a specific thread in `struct thread` and the ones to be shared between all threads in a family in `struct family_shared`.  
Fields that are specific to a single thread's execution — `state`, `chan`, `killed`, `xstate`, `context`, `trapframe`, `kstack_index`, `slot_index` — stay in `struct thread`.  
Fields shared by all threads in a family — `pagetable`, `sz`, `ofile[]`, `cwd`, `vmas[]`, `tcount`, `slot_tracking[]` — move to `struct family_shared`.  
Every thread carries a `family` pointer to the same shared struct.
...
The problem is that page table operations can trigger disk IO (lazy ELF loading via VMAs). A spinlock disables interrupts. If a thread holds a spinlock and goes to sleep waiting for disk IO, the interrupt that would wake it up can never be delivered, which leads to a deadlock.
...
Each thread in the family needs its own user stack and trapframe in the shared address space. The question is how to assign them virtual addresses.
...
The tempting approach is to kill siblings immediately, then do the ELF parsing. We rejected this because if `exec()` fails partway through — bad ELF, OOM during page allocation — your siblings are already dead with no way to undo it. This means that we can't get the process to return to its previous state, on `exec()` failure.

So instead we do all the failure-prone work first: parse the ELF, allocate the new page table, copy the arguments. Only after everything succeeds do we set `no_clone = 1` and kill the siblings. If `exec()` takes the `bad:` path, siblings are still alive and the process is in exactly the state it was before the call.
...
Here, we store the value in a0 register in sscratch register, since we will overwrite it. Then we load the hardcoaded trapframe VA into a0.
...
The problem: `TRAPFRAME` is a hardcoded address. There's only one. This was fine in vanilla xv6, since it had only 1 trapframe per-process. But now, for a multi-threaded xv6, we have slot based trapframe allocation, as mentioned above. At the moment `uservec` fires, all we have is the user register set and `sscratch`. There's no safe way to do a memory lookup to find the current thread's trapframe VA.
...
The two original instructions needed to save `a0` *before* loading `TRAPFRAME` so it wouldn't be overwritten. `csrrw` does both in one instruction with no temporaries.

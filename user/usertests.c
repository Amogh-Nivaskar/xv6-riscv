...
void lazy_copy(char *s)
{
  ...
  unsigned long bad[] = {
      0x3fffffc000,
      // 0x3fffffd000, // slot-based VM: this is USTACK_TOP(0)-PGSIZE = slot 0 user stack, a valid PTE_U page
      0x3fffffe000,
      0x3ffffff000,
      0x4000000000,
      0x8000000000,
...
void lazy_sbrk(char *s)
{
  ...
  // slot-based VM: heap ceiling is SLOT_BASE(0) (guard page of slot 0), not TRAPFRAME.
  // TRAPFRAME-PGSIZE = MAXVA-3*PGSIZE is the user stack page in slot 0 — heap cannot reach it.
  // int n = TRAPFRAME - PGSIZE - (uint64)p;
  int n = SLOT_BASE(0) - PGSIZE - (uint64)p;
  ...
  p = sbrk(PGSIZE);
  // slot-based VM: sbrk returns old sz = SLOT_BASE(0)-PGSIZE, grows to SLOT_BASE(0)
  // if (p < 0 || (uint64)p != TRAPFRAME - PGSIZE)
  if (p < 0 || (uint64)p != SLOT_BASE(0) - PGSIZE)
  {
    // printf("sbrk(%d) returned %p, not expected TRAPFRAME-PGSIZE\n", PGSIZE, p);
    printf("sbrk(%d) returned %p, not expected SLOT_BASE(0)-PGSIZE\n", PGSIZE, p);
    exit(1);
  }
